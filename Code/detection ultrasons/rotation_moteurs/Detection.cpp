...
void Detection::detection_obstacle(int &distance_droite, int &distance_gauche, int &vitesse_droite, int &vitesse_gauche, int i, int M1A, int M2A)
{
  ...
  if (distance_droite > 10 and distance_gauche > 10){
    tourner.toutdroit(vitesse_droite, vitesse_gauche, M1A, M2A); //si les obstacles sont trop loins; il n'y a pas besoin de tourner
  }
  else if (distance_droite > distance_gauche or distance_droite <=10 and distance_gauche <=10){
    tourner.tourneradroite(vitesse_gauche, vitesse_droite, M2A, i); // si la distance du capteur de gauche est inférieure à celle de droite, l'obstacle est proche et à gauche, il faut donc tourner à droite
  }                                                                 // Nous avons aussi décidé qu'en cas de distances égales, le robot tournerai à droite.
           
  else if (distance_droite < distance_gauche){
    tourner.tourneragauche(vitesse_droite, vitesse_gauche, M1A, i); // si la distance du capteur de droite est inférieure à celle de gauche, l'obstacle est proche et à droite, il faut donc tourner à gauche
  }
}
